...
| 长期记忆 | SQLite 持久化，按用户/群隔离；发送 `记住 xxx` 可添加记忆 |
| Token 用量统计 | 记录每次调用的输入/输出 token 与耗时，支持按天聚合 |
| WebUI 面板 | 查看用量趋势、Token 汇总、记忆管理与删除 |

### 学习指令
对机器人发送 `记住 我住在北京`，将作为该用户/群的长期记忆保存，后续对话会参考该记忆生成回复。
